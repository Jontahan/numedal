...
def env_diff(env_a, env_b, iterations, step_size):
    ...
    for ne in range(0, iterations):
        ...
        for state in all_states:
            ...
            total_visits.append((dqn_a.state_visits[state] if state in dqn_a.state_visits else 0) + (dqn_b.state_visits[state] if state in dqn_b.state_visits else 0))

        normalized_visits = {}
        for state in all_states:
            state = torch.from_numpy(state)
            normalized_visits[state] = ((dqn_a.state_visits[state] if state in dqn_a.state_visits else 0) + (dqn_b.state_visits[state] if state in dqn_b.state_visits else 0)) / np.linalg.norm(total_visits)

        for state in all_states:
            state = torch.from_numpy(state)
            action_a = dqn_a.qnet(state).max(0)[1].numpy()
            action_b = dqn_b.qnet(state).max(0)[1].numpy()
            env_diffs.append(1 * normalized_visits[state] if action_a == action_b else 0)
...
for env_pair in envs:
    print(env_diff(env_pair[0], env_pair[1], 10, 5))
